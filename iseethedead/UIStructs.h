...
namespace war3 {
	...
	struct SimpleTopButtonList {
		void*						firstButton;
		uint32_t					firstButtonType;//尚未搞明白
		//...
	};
	...
	struct CChatEditBar {
		...
		uint32_t					flags;			//0xB0 关闭时101101b 打开时101100b
		uint8_t						unk_B4[0x128];	//0xB4
		uint32_t					visible;		//0x1DC	关闭时0 打开时1
	...
	struct CSimpleTop {
		...
		uint8_t unk_8B0[0x4];
		uint32_t						buttonListArrayCount;		//0x8B4
	};

	struct SimpleFrame_DC_Element {
		uint32_t unk_0;		//0x0	=0
		uint32_t unk_4;		//0x4	=this_4
		uint32_t unk_8;		//0x8	=~this_4
	};//sizeof = 0xC
	...
	struct CLayoutFrame {
		void**						vtable;
		uint32_t					pointCount;			//0x4	=9					LayoutFrame
		uint32_t					pTL;				//0x8	=0					LayoutFrame
		uint32_t					pTC;				//0xC	=0					LayoutFrame
		uint32_t					pTR;				//0x10	=0					LayoutFrame
		uint32_t					pL;					//0x14	=0					LayoutFrame
		uint32_t					pC;					//0x18	=0					LayoutFrame
		uint32_t					pR;					//0x1C	=0					LayoutFrame
		uint32_t					pBL;				//0x20	=0					LayoutFrame
		uint32_t					pBC;				//0x24	=0					LayoutFrame
		uint32_t					pBR;				//0x28	=0					LayoutFrame
		uint32_t					unk_2C;				//0x2C	=0					LayoutFrame
		uint32_t					unk_30;				//0x30	=0					LayoutFrame
		uint32_t					unk_34;				//0x34	=0					LayoutFrame
		uint32_t					unk_38;				//0x38	=0					LayoutFrame
		uint32_t					unk_3C;				//0x3C	=this_3C			LayoutFrame	指向自己的指针
		...
		uint32_t					unk_54;				//0x54						LayoutFrame 坐标改变时改变
	...
	struct CSimpleRegion {
		...
		uint32_t					unk_70;				//0x70	0
		CLayoutFrame				parent;				//0x74	0
		uint32_t					unk_78;				//0x78	0
		uint32_t					unk_7C;				//0x7C	0
	...
	struct CSimpleFrame {
		...
		uint32_t					unk_70;				//0x70	=0					SimpleFrame
		uint32_t					unk_74;				//0x74	=0					SimpleFrame
		uint32_t					unk_78;				//0x78	=0					SimpleFrame
		uint32_t					unk_7C;				//0x7C	=0					SimpleFrame	Storm#403 删除
		uint32_t					unk_80;				//0x80	=0					SimpleFrame
		uint32_t					unk_84;				//0x84  =0					CToolTipWar3 初始化在6F609980
		uint32_t					unk_88;				//0x88	=0xFF000000			SimpleFrame 只初始化了第一个BYTE，是BYTE变量？
		uint32_t					unk_8C;				//0x8C	=0					SimpleFrame	flags?
		uint32_t					visible;			//0x90	=0,1				SimpleFrame	ToolTip是0	SimpleFrame是1 显示时是1，隐藏时是0
		uint32_t					needUpdate;			//0x94	=1 bool?			SimpleFrame	在6F609AD0被使用运行完归0	   显示时是1，隐藏时是0
		...
		uint32_t					unk_B8;				//0xB8	=1					SimpleFrame
		uint32_t					unk_BC;				//0xBC	=1					SimpleFrame
		uint32_t					unk_C0;				//0xC0	=1					SimpleFrame
		uint32_t					unk_C4;				//0xC4	=1					SimpleFrame
		uint32_t					unk_C8;				//0xC8	=0					SimpleFrame
		SimpleFrameTextureSettings*	textureSettings;	//0xCC	=0					SimpleFrame, CToolTipWar3
		uint32_t					unk_D0;				//0xD0	=0					SimpleFrame
		uint32_t					unk_D4;				//0xD4	=this_D4			SimpleFrame 指向自己的指针
		uint32_t					unk_D8;				//0xD8	=~this_D4			SimpleFrame
		SimpleFrame_DC				unk_DC;				//0xDC						SimpleFrame 初始化过程见 6F609D14
		uint32_t					unk_118;			//0x118	=0					SimpleFrame
		uint32_t					unk_11C;			//0x11C	=this_11C			SimpleFrame 指向自己的指针
		uint32_t					unk_120;			//0x120	=~this_11C			SimpleFrame
	};//sizeof = 0x124
	...
	struct CSimpleButton {
		...
		uint32_t					clickEventId;		//0x128
		CObserver*					mouseEventObserver;	//0x12C	6F603060 CGameUI*?
		uint32_t					mouseOverEventId;	//0x130
		uint32_t					mouseOutEventId;	//0x134
		uint32_t					currentState;		//0x138
		uint32_t					enabled;			//0x13C
		uint32_t					mouseButtonFlags;	//0x140	=0x10	指定什么鼠标键可以触发按钮
		uint32_t					unk_144;			//0x144
		uint32_t					unk_148;			//0x148
		uint32_t					unk_14C;			//0x14C
	...
	struct CCommandButtonData {
		void**		vtable;			//0x0
		uint32_t	abilityId;		//0x4 物品按钮和基本技能这里是0
		uint32_t	orderId_8;		//0x8
		uint32_t	orderId_C;		//0xC
		uint32_t	flag10;			//0x10
		uint32_t	unk14;			//0x14
		uint32_t	unk18;			//0x18
		HashGroup	agentHash;		//0x1C
		uint32_t	unk24;			//0x24
		uint32_t	isSpell;		//0x28
		char		title[0x100];	//0x2C 长度正确？
		uint8_t		unk12C[0x58];	//0x12C
		uint32_t	unk184;			//0x184
		uint32_t	unk188;			//0x188
		char		tooltip[0x400];	//0x18C 长度正确？
		uint8_t		unk58C[0x8];	//0x58C
		uint32_t	manaCost;		//0x594
		uint32_t	unk598;			//0x598
		uint32_t	displayOrder;	//0x59C ?
		uint32_t	unk5A0;			//0x5A0 英雄技能是2 物品是0
		uint32_t	unk5A4;			//0x5A4 英雄技能是1 物品是0
		uint32_t	unk5A8;			//0x5A8 英雄技能是1 物品是0
		uint32_t	hotkey;			//0x5AC
		uint32_t	unk5B0;			//0x5B0
		uint32_t	unk5B4;			//0x5B4
		uint32_t	unk5B8;			//0x5B8 英雄技能是1 物品是0
		uint32_t	unk5BC;			//0x5BC 英雄技能是1 物品是0
		uint32_t	unk5C0;			//0x5C0 英雄技能是0 物品是1
		uint32_t	unk5C4;			//0x5C4 英雄技能是0 物品是1
		char		iconPath[0x100];//0x5C8 长度正确？
		uint32_t	unk6C8;			//0x6C8
		uint32_t	unk6CC;			//0x6CC
		uint32_t	unk6D0;			//0x6D0
	...
	struct CSimpleFontString {
		...
		uint32_t					flags;				//0xC0	=0x212				
	};//sizeof = 0xC4

	struct CSimpleTexture {
		CSimpleRegion				baseSimpleRegion;	//0x0
		uint32_t					unk_84;				//0x84
	...
	struct CToolTipWar3 {
		...
		uint32_t				unk_154;			//0x154	=4					CToolTipWar3
		uint32_t				unk_158;			//0x158	=4					CToolTipWar3
		uint32_t				unk_15C;			//0x15C	=4					CToolTipWar3
		uint32_t				unk_160;			//0x160	=4					CToolTipWar3
		uint32_t				unk_164;			//0x164	=0					CToolTipWar3
		uint32_t				unk_168;			//0x168	=0					CToolTipWar3
		uint32_t				unk_16C;			//0x16C	=0					CToolTipWar3
		uint32_t				unk_170;			//0x170	=0					CToolTipWar3
		uint32_t				unk_174;			//0x174 =0
		uint32_t				unk_17C;			//0x17C =1
	};
	...
	struct CUnitTip {
		...
		uint32_t				unk_138;			//0x138						单位名更新了要设为1
		uint32_t				updateSize;			//0x13C						单位等级回调更新了要设为1
		uint32_t				unk_140;			//0x140						玩家名更新了要设为1
	...
	struct CLayer {
		...
		uint32_t				unk_C;				//0xC	3
		uint32_t				unk_10;				//0x10	4
		uint32_t				unk_14;				//0x14	5
		uint32_t				unk_18;				//0x18	6 = offset unk_18
		uint32_t				unk_1C;				//0x1C	7 = ~offset unk_18
		uint32_t				unk_20;				//0x20	8 
		uint32_t				unk_24;				//0x24	9
		uint32_t				unk_28;				//0x28	10
		uint32_t				unk_2C;				//0x2C	11
		uint32_t				unk_30;				//0x30	12
		uint32_t				unk_34;				//0x34	13
		uint32_t				unk_38;				//0x38	14
		uint32_t				unk_3C;				//0x3C	15
		uint32_t				unk_40;				//0x40	16
		uint32_t				unk_44;				//0x44	17
		uint8_t					unk_48[0x68];		//0x48
		uint32_t				flag_B0;			//0xB0
	}; //sizeof = 0xB4
	...
	struct CScreenFrame {
		...
		uint32_t				vector_170_0;		//0x170
		uint32_t				vector_170_4;		//0x174
		uint32_t				vector_170_8;		//0x178
		uint32_t				vector_170_C;		//0x17C
	};//sizeof = 180

	struct CGlueMgr {
		CScreenFrame			baseScreenFrame;	//0x0
		uint32_t				unk_180;			//0x180 = 17
		void*					currentFrame;		//0x184
		uint32_t				unk_188;			//0x188
		uint8_t					unk_18C[0x4];		//0x18C
		uint32_t				unk_190;			//0x190
		uint32_t				unk_194;			//0x194
		uint32_t				unk_19C;			//0x19C
		uint32_t				unk_1A0;			//0x1A0 = 17
		uint8_t					unk_1A4[0x4];		//0x1A4
		uint32_t				unk_1A8;			//0x1A8
	};//sizeof = 1AC
	...
	struct CSpriteFrame {
		...
		float					unk_168;			//0x168
		float					unk_16C;			//0x16C
		uint32_t				unk_170;			//0x170
		uint32_t				unk_174;			//0x174
		uint32_t				unk_178;			//0x178
		uint32_t				unk_17C;			//0x17C
		CSpriteFrame_180		struct_180;			//0x180
		float					unk_19C;			//0x19C = 1.0
		uint32_t				unk_1A0;			//0x1A0
		uint32_t				unk_1A4;			//0x1A4
		float					unk_1A8;			//0x1A8 = 0.5
		uint32_t				unk_1AC;			//0x1AC = 1
	}; //sizeof = 0x1B0
	...
	struct CHighlightFrame {
		CFrame					baseFrame;			//0x0
		uint32_t				unk_168;			//0x168 6F624080
		uint32_t				unk_16C;			//0x16C
		uint32_t				unk_170;			//0x170 = 1
		float					unk_174;			//0x174
		float					unk_178;			//0x178
		float					unk_17C;			//0x17C
		float					unk_180;			//0x180
		...
		uint32_t				unk_28C;			//0x28C = 0xFFFFFFFF
		float					unk_290;			//0x290
		uint32_t				unk_294;			//0x294 = 2
		float					unk_298;			//0x298
		float					unk_29C;			//0x29C = 0.01
		uint32_t				unk_2A0;			//0x2A0 = 2
	}; //sizeof = 0x2A4

	struct CControl {
		CSpriteFrame			baseSprintFrame;	//0x0
		uint32_t				unk_1B0;			//0x1B0
		CHighlightFrame*		highlightFrame;		//0x1B4
		uint32_t				unk_1B8;			//0x1B8
		CBackDropFrame*			unk_1BC;			//0x1BC
		CBackDropFrame*			unk_1C0;			//0x1C0
		CBackDropFrame*			unk_1C4;			//0x1C4
		CBackDropFrame*			unk_1C8;			//0x1C8
		uint32_t				unk_1CC;			//0x1CC 初始化6F601D60
		uint32_t				unk_1D0;			//0x1D0 初始化6F601D60
		uint32_t				unk_1D4;			//0x1D4 初始化6F601DE0
		uint32_t				unk_1D8;			//0x1D8	初始化6F601DE0
		uint32_t				unk_1DC;			//0x1DC
		uint32_t				unk_1E0;			//0x1E0 = 1
	}; //sizeof = 0x1E4
	...
	struct CTextFrame {
		...
		char*						text;					//0x1E8
		uint32_t					unk_1EC;				//0x1EC
		TextFrameShadowSettings*	shadowSettings;			//0x1F0
		uint32_t					color;					//0x1F4 = 0xFFFFFFFF ARGB
		uint32_t					unk_1F8;				//0x1F8 = 0xFFFFFFFF
		uint32_t					unk_1FC;				//0x1FC = 0xFFFFFFFF
		uint32_t					unk_200;				//0x200 = 0xFF000000
		uint32_t					unk_204;				//0x204
		float						unk_208;				//0x208
		float						unk_20C;				//0x20C
		TextFrameFontSettings		fontSettings;			//0x210
		float						unk_218;				//0x218
		uint32_t					unk_21C;				//0x21C
		uint32_t					unk_220;				//0x220
		uint32_t					unk_224;				//0x224 = 1
		uint32_t					unk_228;				//0x228
		uint32_t					unk_22C;				//0x22C = 1
		uint32_t					unk_230;				//0x230 = 1
		uint8_t						unk_234[0x8];			//0x234 
		uint32_t					needUpdate;				//0x23C
	}; //sizeof = 0x240
