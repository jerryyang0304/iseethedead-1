...
const char* LIC_NAME = "iseethedead.lic";
...
namespace vmplic {
	...
	bool checkLicense(VOIDCALLBACK a) {
		char* serial = read_serial(LIC_NAME);
		int res = VMProtectSetSerialNumber(serial);
		delete[] serial;
		bool ret = false;
		
		if (res == SERIAL_STATE_SUCCESS) {
			ret = true;
			a();
		}
		else {
			...
			MessageBoxW(NULL, msg, L":(", MB_ICONSTOP | MB_APPLMODAL | MB_TOPMOST);
		}
		return ret;
	};
}